...
// }, []);
    ...
    const sendMessage = async () => {
     ...
     useEffect(() => {
        const socket = io( `${process.env.REACT_APP_SERVER_URL}`);
        socket.emit('joinChat', { userId: localStorage.getItem("loggedInUserId") });
        socket.emit('setup', { userId: localStorage.getItem("loggedInUserId") });
        socket.on('connected', () => {
        ...
        socket.on('reconnect_attempt', (attempt) => {
            ...
            toast.info(`Attempting to reconnect... Attempt ${attempt}`);
        });
...
const filteredUsers = users.filter(user =>
    ...
    return (
        <div className="chat-container">
            <button onClick={() => navigate(-1)} className='back'  style={{ 
  ...
  <div className='searchbar' style={{ position: "relative", width: "250px" }}>
    <input
      type="search"
  ...
  <center className="text-secondary m-5 shadow-sm">
    <i>No Teammates are found...!</i>
         ...
         <Tooltip anchorSelect='.file-upload-icon' place='top'>Attach file</Tooltip>
         <style>{`
